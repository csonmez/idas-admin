...
const jsDateToDateValue = (date: Date | string | undefined) => {
...
const calendarYearRange = computed(() =>
    createYearRange({
        ...
        end: parseDate(`${currentYear + 10}-12-31`)
    })
)
...
const parseApiDateToDate = (dateStr: string | Date | undefined): Date | undefined => {
...
const {
    ...
    () =>
        useRequest<Project>(`/manager/projects/${projectId}`, {
            method: 'GET'
        }),
    { server: true }
)
...
const initialValues = computed(() => {
    ...
    return {
        ...
        totalBudget: p.totalBudget != null ? Number(p.totalBudget) : 0,
        year: p.year ?? currentYear,
        startDate: parseApiDateToDate(p.startDate),
        endDate: parseApiDateToDate(p.endDate)
    }
})

watchEffect(() => {
    const ptId = initialValues.value.projectTypeId
    if (ptId && project.value?.projectType) {
        selectedProjectTypeId.value = ptId
        const pt = project.value.projectType
        if (!projectTypes.value.some((p) => p.id === pt.id)) {
            projectTypes.value = [{ id: pt.id, name: pt.name }, ...projectTypes.value]
        }
    }
})
...
watch(project, (p) => {
    if (p) {
        startDateCalendarValue.value = jsDateToDateValue(p.startDate)
        endDateCalendarValue.value = jsDateToDateValue(p.endDate)
    }
}, { immediate: true })
...
const onSubmit: SubmissionHandler<GenericObject, GenericObject, unknown> = async (values, { resetForm }) => {
    try {
        ...
        await useRequest<Project>(`/manager/projects/${projectId}`, {
            method: 'PATCH',
            body
        })

        isLoading.value = false
        navigateTo(`/dashboard/projects/${projectId}?success=updated`)
        resetForm()
    } catch (error) {
    ...
    <div class="relative w-full gap-4 p-4 lg:gap-6 lg:p-6">
        ...
        <div v-if="pending" class="flex justify-center items-center min-h-[40vh]">
            <div class="flex flex-col items-center gap-4">
                <Loader class="w-12 h-12 animate-spin text-muted-foreground" />
        ...
        <Card v-else-if="error" class="max-w-md mx-auto">
            <CardHeader>
                <CardTitle>Proje yüklenemedi</CardTitle>
                <CardDescription>{{ error?.message || 'Bilinmeyen bir hata oluştu' }}</CardDescription>
            </CardHeader>
            <CardContent>
                <Button variant="outline" @click="refresh()">Tekrar Dene</Button>
            </CardContent>
        </Card>
        ...
        <Card v-else-if="project">
            <CardHeader>
                <CardTitle>Proje Güncelle</CardTitle>
            </CardHeader>
                ...
                <Form class="grid gap-4" :validation-schema="formSchema" :initial-values="initialValues" @submit="onSubmit">
                    ...
                    <FormField v-slot="{ componentField }" class="grid gap-2" name="startDate">
                        <FormItem>
                            ...
                            <FormControl>
                                <Popover v-slot="{ close }" :modal="true">
                                    <PopoverTrigger as-child>
                                    ...
                                    <PopoverContent class="w-auto p-0" align="start" @pointer-down-outside="(e: { preventDefault: () => void }) => e.preventDefault()">
                                        <Calendar
                                            locale="tr-TR"
                    ...
                    <FormField v-slot="{ componentField }" class="grid gap-2" name="endDate">
                        <FormItem>
                            ...
                            <FormControl>
                                <Popover v-model:open="endDatePopoverOpen" :modal="true">
                                    <PopoverTrigger as-child>
                                        <Button variant="outline" :class="cn('w-full justify-start text-left font-normal', !componentField.modelValue && 'text-muted-foreground')">
                                            <CalendarIcon class="mr-2 h-4 w-4" />
                                            {{
        ...
        <!-- Proje bulunamadı -->
        <Card v-else-if="!pending && !error" class="max-w-md mx-auto">
            <CardHeader>
                <CardTitle>Proje bulunamadı</CardTitle>
                <CardDescription>Aradığınız proje bulunamadı veya erişim izniniz yok.</CardDescription>
            </CardHeader>
            <CardContent>
                <Button variant="outline" @click="navigateTo('/dashboard/projects')">Projelere Dön</Button>
            </CardContent>
        </Card>
    </div>
</template>
