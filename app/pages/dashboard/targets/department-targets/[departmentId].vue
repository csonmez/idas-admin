...
import type { Target, TargetItems, TargetIndicator, DepartmentTarget, Department } from '~/types'
...
const { data, pending, error, refresh } = await useAsyncData(
    `department-target-detail-${departmentId.value}`,
    async () => {
        const [departmentRes, targetsRes, departmentTargetsRes] = await Promise.all([
            useRequest<Department>(`/manager/departments/${departmentId.value}`),
            useRequest<Target[] | { rows: Target[] }>('/manager/targets', { query: { onlyData: true } }),
            useRequest<DepartmentTarget[]>(`/manager/department-targets`, {
                query: { departmentId: departmentId.value, includeAllYears: true, onlyData: true }
            })
        ])

        const targetRows = Array.isArray(targetsRes) ? targetsRes : ((targetsRes as { rows: Target[] })?.rows ?? [])
        const deptTargets = Array.isArray(departmentTargetsRes) ? departmentTargetsRes : []
        return {
            department: departmentRes,
            targets: targetRows,
            departmentTargets: deptTargets
        }
    },
    { watch: [departmentId] }
)
...
const currentDepartmentTarget = computed(() => {
    const year = Number(selectedYear.value)
    return data.value?.departmentTargets?.find((dt: DepartmentTarget) => dt.year === year) ?? null
})

const getIndicatorValue = (targetId: string, indicatorId: string, type: 'target' | 'actual') => {
    const dt = currentDepartmentTarget.value
...
watch(
    [data, selectedYear, () => currentDepartmentTarget.value],
    () => {
        if (!targetStructure.value?.items) return
...
const handleSave = async () => {
    if (!currentDepartmentTarget.value || !targetStructure.value?.items) return
    try {
        ...
        await useRequest(`/manager/department-targets/${currentDepartmentTarget.value.id}`, {
            method: 'PATCH',
            body: { data: targetData }
        })

        $toast({ title: 'Bölüm hedefleri başarıyla güncellendi.' })
        await refresh()
    } catch (err) {
        console.error(err)
        $toast({
...
const handleComplete = async () => {
    if (!currentDepartmentTarget.value) return
    try {
        isCompleteLoading.value = true
        await useRequest(`/manager/department-targets/${currentDepartmentTarget.value.id}/status`, {
            method: 'PATCH',
            body: { status: 'COMPLETED' }
        })
        $toast({ title: 'Bölüm hedefleri başarıyla onaylandı.' })
...
const isCompleted = computed(() => currentDepartmentTarget.value?.status === 'COMPLETED')
const isInProgress = computed(() => currentDepartmentTarget.value?.status === 'IN_PROGRESS')
        ...
        <div class="mx-auto w-full max-w-7xl">
            <div class="mb-6">
                <div class="flex items-center gap-3 mb-1">
                    ...
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Hedef Yönetimi – {{ data?.department?.name ?? 'Bölüm' }}</h1>
                </div>
                <p class="text-sm text-gray-600 dark:text-gray-400 ml-11 mb-3">Bölüm hedeflerini yıl bazında görüntüleyin ve düzenleyin</p>
                <div
                ...
                >
                    ...
                    <div v-if="currentDepartmentTarget && Number(selectedYear) === currentYear" class="flex items-center gap-2">
            ...
            <AlertDialog v-model:open="isOpenApproveDialog">
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Bölüm hedeflerini onaylamak istediğinize emin misiniz?</AlertDialogTitle>
                        <AlertDialogDescription> {{ data?.department?.name }} bölümünün {{ selectedYear }} yılı Ar-Ge hedeflerini onaylamak için "Onayla" butonuna tıklayınız. </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                ...
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            {{ data?.department?.name }} bölümünün {{ selectedYear }} yılı hedef onayı geri alınacak. Hedefler yeniden düzenlenebilir hale gelecektir.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>İptal</AlertDialogCancel>
            ...
            <div v-else-if="processedData.length > 0 && currentDepartmentTarget" class="space-y-6">
            ...
            <Card v-else-if="processedData.length > 0 && !currentDepartmentTarget">
                <CardContent class="pt-6">
