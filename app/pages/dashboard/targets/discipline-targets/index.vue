...
import type { Discipline, DisciplineTarget } from '@/types'
...
interface DisciplineWithDepartment extends Omit<Discipline, 'department'> {
    department?: { id?: string; name?: string; faculty?: { id?: string; name?: string } }
    faculty?: { id?: string; name?: string }
    head?: string
    headName?: string
    disciplineHead?: { name?: string; surname?: string; title?: string } | string
    userAcademicUnits?: Array<{ user?: { name?: string; surname?: string; title?: string } }>
}

interface DisciplineWithTargetStatus {
    discipline: DisciplineWithDepartment
    target2026: DisciplineTarget | null
}
...
const normalizeToArray = <T,>(response: unknown): T[] => {
    ...
    if (response && typeof response === 'object') {
        ...
        if (Array.isArray(r.disciplineTargets)) return r.disciplineTargets as T[]
        if (Array.isArray(r.result)) return r.result as T[]
    }
    return []
}

const { data, pending } = await useAsyncData('discipline-targets-page', async () => {
    const [disciplinesResponse, disciplineTargetsResponse] = await Promise.all([
        useRequest('/manager/disciplines', {
            method: 'GET',
            query: {
                select: 'id,name,department,faculty',
                onlyData: true,
                useDepartment: true,
                useFaculty: true,
                enableDisciplineHeadAccess: true,
                isAll: true,
                sortBy: 'name',
                sortOrder: 'asc'
            }
        }),
        useRequest('/manager/discipline-targets', {
            method: 'GET',
            query: {
                year: 2026,
                onlyData: true
    ...
    const disciplines = normalizeToArray<DisciplineWithDepartment>(disciplinesResponse)
    const disciplineTargets = normalizeToArray<DisciplineTarget>(disciplineTargetsResponse)

    return { disciplines, disciplineTargets }
})

const tableData = computed<DisciplineWithTargetStatus[]>(() => {
    const disciplines = data.value?.disciplines ?? []
    const disciplineTargets = data.value?.disciplineTargets ?? []

    return [...disciplines]
        .sort((a, b) => (a.name ?? '').localeCompare(b.name ?? '', 'tr'))
        .map((discipline) => {
            const target2026 = disciplineTargets.find((dt) => dt.disciplineId === discipline.id) ?? null
            return { discipline, target2026 }
        })
})
...
const uniqueFaculties = computed(() => {
    ...
    for (const row of tableData.value) {
        ...
        const facultyId = disc.faculty?.id ?? disc.department?.faculty?.id
        const facultyName = disc.faculty?.name ?? disc.department?.faculty?.name ?? 'Belirtilmemiş'
...
const filteredTableData = computed<DisciplineWithTargetStatus[]>(() => {
...
const statCards = computed(() => {
    const s = stats.value
    return [
        { key: 'total', count: s.total, label: 'Toplam Anabilim Dalı', cardClass: 'bg-card', colorClass: '' },
        {
            key: 'completed',
            count: s.completed,
            label: 'Tamamlandı',
...
const columnHelper = createColumnHelper<DisciplineWithTargetStatus>()
const columns = [
    columnHelper.accessor((row) => row.discipline.name, {
        id: 'name',
        header: () => h('div', { class: 'p-2' }, 'Anabilim Dalı Adı'),
        cell: ({ row }) =>
            h(
                'div',
                {
    ...
    columnHelper.display({
        ...
        cell: ({ row }) => {
            const departmentName = row.original.discipline.department?.name ?? 'Belirtilmemiş'
            return h(
                'div',
                {
    ...
    columnHelper.display({
        ...
        cell: ({ row }) => {
            const disc = row.original.discipline
            const facultyName = disc.faculty?.name ?? disc.department?.faculty?.name ?? 'Belirtilmemiş'
            return h(
                'div',
                {
    ...
    columnHelper.display({
        id: 'head',
        header: () => h('div', { class: 'p-2 text-left' }, 'Anabilim Dalı Başkanı'),
        cell: ({ row }) => {
            const disc = row.original.discipline
            const dh = disc.disciplineHead
            let headName: string
            if (dh) {
                headName = typeof dh === 'string' ? dh : formatUserName(dh as Parameters<typeof formatUserName>[0]) || 'Belirtilmemiş'
            } else {
                const headUser = disc.userAcademicUnits?.[0]?.user
                headName = headUser ? formatUserName(headUser as Parameters<typeof formatUserName>[0]) || 'Belirtilmemiş' : (disc.head ?? disc.headName ?? 'Belirtilmemiş')
            }
            return h(
                'div',
                {
    ...
    columnHelper.display({
        ...
        cell: ({ row }) => {
            const id = row.original.discipline.id
            return h(
                NuxtLink,
                {
                    to: `/dashboard/targets/discipline-targets/${id}`,
        ...
        <Card>
            <CardHeader>
                <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div>
                        <CardTitle>Anabilim Dalı 2026 Hedef Durumları</CardTitle>
                        <p class="text-sm text-muted-foreground mt-1">Anabilim dallarının 2026 yılı Ar-Ge hedef giriş durumları</p>
                    </div>
