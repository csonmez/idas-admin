...
import type { Target, TargetItems, TargetIndicator, DisciplineTarget, Discipline } from '~/types'
...
const { data, pending, error, refresh } = await useAsyncData(
    `discipline-target-detail-${disciplineId.value}`,
    async () => {
        const [disciplineRes, targetsRes, disciplineTargetsRes] = await Promise.all([
            useRequest<Discipline>(`/manager/disciplines/${disciplineId.value}`),
            useRequest<Target[] | { rows: Target[] }>('/manager/targets', { query: { onlyData: true } }),
            useRequest<DisciplineTarget[]>(`/manager/discipline-targets`, {
                query: { disciplineId: disciplineId.value, includeAllYears: true, onlyData: true }
            })
        ])

        const targetRows = Array.isArray(targetsRes) ? targetsRes : ((targetsRes as { rows: Target[] })?.rows ?? [])
        const discTargets = Array.isArray(disciplineTargetsRes) ? disciplineTargetsRes : []
        return {
            discipline: disciplineRes,
            targets: targetRows,
            disciplineTargets: discTargets
        }
    },
    { watch: [disciplineId] }
)
...
const currentDisciplineTarget = computed(() => {
    const year = Number(selectedYear.value)
    return data.value?.disciplineTargets?.find((dt: DisciplineTarget) => dt.year === year) ?? null
})

const getIndicatorValue = (targetId: string, indicatorId: string, type: 'target' | 'actual') => {
    const dt = currentDisciplineTarget.value
    if (!dt?.data) return null
    const cat = dt.data.find((d: { targetId: string }) => d.targetId === targetId)
...
watch(
    [data, selectedYear, () => currentDisciplineTarget.value],
    () => {
        if (!targetStructure.value?.items) return
...
const handleSave = async () => {
    if (!currentDisciplineTarget.value || !targetStructure.value?.items) return
    try {
        ...
        await useRequest(`/manager/discipline-targets/${currentDisciplineTarget.value.id}`, {
            method: 'PATCH',
            body: { data: targetData }
        })

        $toast({ title: 'Anabilim dalı hedefleri başarıyla güncellendi.' })
        await refresh()
    } catch (err) {
        console.error(err)
        $toast({
...
const handleComplete = async () => {
    if (!currentDisciplineTarget.value) return
    try {
        isCompleteLoading.value = true
        await useRequest(`/manager/discipline-targets/${currentDisciplineTarget.value.id}/status`, {
            method: 'PATCH',
            body: { status: 'COMPLETED' }
        })
        $toast({ title: 'Anabilim dalı hedefleri başarıyla onaylandı.' })
...
const isCompleted = computed(() => currentDisciplineTarget.value?.status === 'COMPLETED')
const isInProgress = computed(() => currentDisciplineTarget.value?.status === 'IN_PROGRESS')
        ...
        <div class="mx-auto w-full max-w-7xl">
            <div class="mb-6">
                <div class="flex items-center gap-3 mb-1">
                    ...
                    <h1 class="text-2xl font-bold text-gray-900 dark:text-gray-100">Hedef Yönetimi – {{ data?.discipline?.name ?? 'Anabilim Dalı' }}</h1>
                </div>
                <p class="text-sm text-gray-600 dark:text-gray-400 ml-11 mb-3">Anabilim dalı hedeflerini yıl bazında görüntüleyin ve düzenleyin</p>
                <div
                ...
                >
                    ...
                    <div v-if="currentDisciplineTarget && Number(selectedYear) === currentYear" class="flex items-center gap-2">
            ...
            <AlertDialog v-model:open="isOpenApproveDialog">
                <AlertDialogContent>
                    <AlertDialogHeader>
                        <AlertDialogTitle>Anabilim dalı hedeflerini onaylamak istediğinize emin misiniz?</AlertDialogTitle>
                        <AlertDialogDescription>
                            {{ data?.discipline?.name }} anabilim dalının {{ selectedYear }} yılı Ar-Ge hedeflerini onaylamak için "Onayla" butonuna tıklayınız.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
            ...
            <AlertDialog v-model:open="isOpenRevokeDialog">
                <AlertDialogContent>
                    <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                            {{ data?.discipline?.name }} anabilim dalının {{ selectedYear }} yılı hedef onayı geri alınacak. Hedefler yeniden düzenlenebilir hale gelecektir.
                        </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
                        <AlertDialogCancel>İptal</AlertDialogCancel>
            ...
            <div v-else-if="processedData.length > 0 && currentDisciplineTarget" class="space-y-6">
            ...
            <Card v-else-if="processedData.length > 0 && !currentDisciplineTarget">
                <CardContent class="pt-6">
