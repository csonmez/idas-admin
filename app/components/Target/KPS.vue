...
const emit = defineEmits(['next', 'update:modelValue', 'getHistoryData', 'update:hasModifications'])

const debouncedModelValue = ref(props.modelValue)
const isOnlyShown = computed(() => props.isOnlyShown)
const kpsIndicators = computed(() => (props.from === 'user' ? props.indicators?.filter((i) => i.isAcademicianIndicator) : props.indicators))
...
<template>
    <div class="mx-auto">
        <Card>
            <CardHeader>
                <CardTitle>1. Araştırma Kapasitesi</CardTitle>
            </CardHeader>
            <CardContent class="p-5">
                <form @submit="onSubmit">
                    <table class="w-full">
                        ...
                        <tbody>
                            <tr v-for="item in kpsIndicators" :key="item.id" class="border-b">
                                <td class="py-4 text-sm">
                                    <span class="font-bold">
                                        {{ !item.subIndicatorType || item.subIndicatorType === 'a' ? 'KPS' + item.indicatorGroup + '.' : '' }}
                                    </span>
